...
// ProcessSourceMapContent analyses a JavaScript source map file for dependency-
// confusion vulnerabilities using two complementary strategies:
//
//  1. Sources-path scan — inspects every entry in the "sources" array for
//     "node_modules/<pkg>" path segments and queues a VerifyPackage job for
//     each unique package name found.  This catches packages that are present
//     in the bundle graph even if their require()/import was tree-shaken away
//     or is deeply nested inside a chunk that was not directly fetched.
//
//  2. SourcesContent re-processing — feeds each non-null "sourcesContent" entry
//     (the original, pre-minification JS of each source file) back through the
//     normal JS extraction pipeline (regex or AST depending on --deep-scan).
//     Because these strings are the unminified originals they yield far more
//     accurate require()/import matches than the minified bundle text.
func (p *Processor) ProcessSourceMapContent(sourceURL string, body []byte) error {
	p.logger.Debugf("Processing source map from %s (%d bytes)", sourceURL, len(body))

	sm, err := input.ParseSourceMap(body)
	if err != nil {
		p.logger.Warnf("Failed to parse source map from '%s': %v", sourceURL, err)
		return err
	}

	// Strategy 1 — extract package names directly from node_modules/ paths in
	// the sources array.  This works even for packages whose code was
	// tree-shaken from the final bundle.
	packages := input.PackagesFromSources(sm.Sources)
	p.logger.Debugf("Source map '%s': %d source(s), %d node_modules package(s) found in paths",
		sourceURL, len(sm.Sources), len(packages))

	for _, pkg := range packages {
		normalized := p.normalizePackageName(pkg)
		if p.isPackageWorthChecking(normalized) {
			if _, loaded := p.checkedPackages.LoadOrStore(normalized, true); !loaded {
				verifyJob := Job{
					Input:     normalized,
	...
	// Strategy 2 — re-run the JS extraction pipeline on each sourcesContent
	// entry.  The entries are the original (unminified) source files, so the
	// regex and AST scanners produce far cleaner results than they would on the
	// minified bundle text.
	contents := input.SourceContents(sm)
	p.logger.Debugf("Source map '%s': processing %d sourcesContent entr(ies) as JS", sourceURL, len(contents))

	for _, content := range contents {
		// ProcessJSFileContent respects the --deep-scan flag automatically.
		if err := p.ProcessJSFileContent(sourceURL, []byte(content)); err != nil {
			p.logger.Debugf("Error processing sourcesContent entry from '%s': %v", sourceURL, err)
		}
	}
